...
public class GiocoServiceImpl implements GiocoService {
    ...
    public Gioco inizializzaPartita() throws InizializzaPartitaException {
        ...
        // Menu iniziale per scegliere tra nuova partita o caricamento
     ...
     * @throws InizializzaPartitaException se si verifica un errore durante il caricamento.
     */
    ...
    private Gioco avviaNuovaPartita() throws InizializzaPartitaException {
        ...
        // Step 8: Distribuzione delle armate iniziali (stampa ASCII art e pausa)
        OutputUtils.println("\r\n"
    ...
    /**
     * Verifica se il giocatore ha conquistato tutti i territori di un continente specifico.
     *
     * @param giocatore      il giocatore da verificare.
     * @param nomeContinente il nome del continente.
     * @param gioco          l'oggetto Gioco corrente.
     * @return true se il giocatore controlla tutti i territori del continente, false altrimenti.
     */
     ...
     * @param giocatore il giocatore corrente.
     * @param gioco     l'oggetto Gioco corrente.
     * @return true se il turno è stato completato correttamente, false se il gioco termina.
     */
    public boolean turnoGiocatore(Giocatore giocatore, Gioco gioco) {
        TurnState turnState = gioco.getCurrentTurnState();
        ...
        if (turnState.getCurrentPhase() == null) {
            turnState.setCurrentPhase(TurnState.TurnPhase.START_TURN);
        }
        ...
        while (!turnState.isTurnoTerminato()) {
            switch (turnState.getCurrentPhase()) {
                case START_TURN:
                    ...
                    FileServiceImpl.getInstance().writeLog("Giocatore " + giocatore.getNome().toUpperCase() + " riceve " + armateTotali + " armate (Territori: " + numeroTerritori + ", Continenti: " + numeroContinenti + ").");
                    ...
                    turnState.setCurrentPhase(TurnState.TurnPhase.DISTRIBUTE_ARMIES);
                    break;

                case DISTRIBUTE_ARMIES:
                    distribuzioneArmate(giocatore, turnState, gioco);
                    turnState.setCurrentPhase(TurnState.TurnPhase.MENU);
                    break;
                        ...
                        switch (scelta) {
                            ...
                            case 5:
                                try {
                                    ...
                                    continueMenu = false;
                                    OutputUtils.println("\nTurno di " + giocatore.getNome().toUpperCase() + " terminato dopo lo spostamento delle armate.", OutputUtils.ANSI_BRIGHT_YELLOW, OutputUtils.ANSI_BOLD);
                                    FileServiceImpl.getInstance().writeLog("Turno di " + giocatore.getNome().toUpperCase() + " terminato dopo lo spostamento delle armate.");
                                    FileServiceImpl.getInstance().writeLog(""); // Riga vuota per separare le azioni
                                    try {
                                        Thread.sleep(5000);
                                    } catch (InterruptedException e) {
        ...
        gioco.setCurrentTurnState(new TurnState());
        return true;
    }
     ...
     *
     * @param giocatore il giocatore corrente.
     * @return il territorio selezionato, o null se non ci sono territori disponibili.
     */
    private Territorio selezionaTerritorioPerAttacco(Giocatore giocatore) {
        // Filtra i territori controllati con almeno 2 armate
        List<Territorio> territoriAttaccabili = giocatore.getTerritori_controllati().stream().filter(t -> t.getNumeroArmate() >= 2).collect(Collectors.toList());
        ...
        OutputUtils.print("\nScelta: ", OutputUtils.ANSI_CYAN, OutputUtils.ANSI_BOLD);
        int indiceTerritorio = SingletonMain.getInstance().readIntegerUntilPossibleValue(IntStream.rangeClosed(1, territoriAttaccabili.size()).boxed().toArray(Integer[]::new)) - 1;
    ...
    private void attaccoGiocatore(Giocatore giocatore, Gioco gioco) {
        ...
        while (!attaccoTerminato) {
            ...
            List<Territorio> territoriAttaccabili = territorioAttaccante.getTerritoriAdiacenti().stream().filter(t -> !t.getGiocatore().equals(giocatore) && t.getNumeroArmate() > 0).collect(Collectors.toList());
            ...
            OutputUtils.print("\nScelta: ", OutputUtils.ANSI_CYAN, OutputUtils.ANSI_BOLD);
            int indiceTerritorioDifensore = SingletonMain.getInstance().readIntegerUntilPossibleValue(IntStream.rangeClosed(1, territoriAttaccabili.size()).boxed().toArray(Integer[]::new)) - 1;
            System.out.println();
            ...
            FileServiceImpl.getInstance().writeLog("Giocatore " + giocatore.getNome().toUpperCase() + " attacca il territorio " + territorioDifensore.getNome() + " posseduto da " + territorioDifensore.getGiocatore().getNome().toUpperCase());
            FileServiceImpl.getInstance().writeLog("");
            ...
            int dadiAttacco = SingletonMain.getInstance().readIntegerUntilPossibleValue(IntStream.rangeClosed(1, maxDadiAttacco).boxed().toArray(Integer[]::new));
            ...
            boolean territorioConquistato = attacca(giocatore, gioco, territorioAttaccante, territorioDifensore, dadiAttacco, dadiDifesa);
            ...
            if (territorioConquistato) {
                ...
                FileServiceImpl.getInstance().writeLog("Giocatore " + giocatore.getNome().toUpperCase()+ " ha spostato " + armateSpostate + " armate da " + territorioAttaccante.getNome()+ " a " + territorioDifensore.getNome() + " dopo averlo conquistato.");
                FileServiceImpl.getInstance().writeLog(""); // Riga vuota per separare le azioni
            ...
            while (!inputValido) {
                OutputUtils.print("\nVuoi continuare ad attaccare? (S/N): ", OutputUtils.ANSI_CYAN, OutputUtils.ANSI_BOLD);
                String risposta = SingletonMain.getInstance().readString();
                if (risposta.equalsIgnoreCase("s")) {
                    ...
                    OutputUtils.println("\nInput non valido. Inserisci 'S' per sì o 'N' per no.", OutputUtils.ANSI_RED, OutputUtils.ANSI_BOLD);
                }
            }
        }
    }
     ...
     */
    private boolean attacca(Giocatore giocatore, Gioco gioco, Territorio territorioAttaccante, Territorio territorioDifensore, int dadiAttacco, int dadiDifesa) {
        // Stampa lo stato iniziale
        System.out.println();
        OutputUtils.println("Attacco dal territorio " + territorioAttaccante.getNome() + " (armate: " + territorioAttaccante.getNumeroArmate() + ")", OutputUtils.ANSI_RED, OutputUtils.ANSI_BOLD);
        OutputUtils.println("Difesa del territorio " + territorioDifensore.getNome() + " (armate: " + territorioDifensore.getNumeroArmate() + ")", OutputUtils.ANSI_BLUE, OutputUtils.ANSI_BOLD);
        ...
        System.out.println();
        OutputUtils.println("L'attaccante ha lanciato i dadi: " + risultatiAttacco, OutputUtils.ANSI_RED, OutputUtils.ANSI_BOLD);
        
        // Il difensore lancia i dadi
        List<Integer> risultatiDifesa = lanciaDadi(dadiDifesa);
        OutputUtils.println("Il difensore ha lanciato i dadi: " + risultatiDifesa, OutputUtils.ANSI_BLUE, OutputUtils.ANSI_BOLD);
        ...
        OutputUtils.println("\nRisultato dell'attacco: " + armatePerseAttaccante + " armate perse dall'attaccante, " + armatePerseDifensore + " armate perse dal difensore.", OutputUtils.ANSI_BRIGHT_YELLOW, OutputUtils.ANSI_BOLD);

        // Logging del risultato dell'attacco
        FileServiceImpl.getInstance().writeLog("\nRisultato dell'attacco: " + armatePerseAttaccante + " armate perse dall'attaccante, " + armatePerseDifensore + " armate perse dal difensore.");
        FileServiceImpl.getInstance().writeLog("");

        // Se il difensore ha perso tutte le armate, il territorio è conquistato
        if (territorioDifensore.getNumeroArmate() == 0) {
            OutputUtils.println("\nIl territorio " + territorioDifensore.getNome() + " è stato conquistato!", OutputUtils.ANSI_BRIGHT_YELLOW, OutputUtils.ANSI_BOLD);
            System.out.println();

            // Logging della conquista del territorio
            FileServiceImpl.getInstance().writeLog("Giocatore " + giocatore.getNome().toUpperCase() + " ha conquistato il territorio " + territorioDifensore.getNome() + " da " + territorioDifensore.getGiocatore().getNome().toUpperCase());
            ...
            giocatore.incrementaTerritoriConquistatiNelTurno(); // Aggiunge il territorio conquistato ai territori del giocatore
            ...
            if (!giocatore.getHaRicevutoCartaBonus()) {
                ...
                if (nuovaCarta != null) {
                    ...
                    OutputUtils.println("Hai ricevuto una nuova carta: " + nuovaCarta.getTipo() + (nuovaCarta.getTerritorio() != null ? " - " + nuovaCarta.getTerritorio().getNome() : ""), OutputUtils.ANSI_GREEN, OutputUtils.ANSI_BOLD);
                    System.out.println();
                    // Log dell'evento
                    FileServiceImpl.getInstance().writeLog("Giocatore " + giocatore.getNome().toUpperCase() + " ha ricevuto una carta: " + nuovaCarta.getTipo() + (nuovaCarta.getTerritorio() != null ? " - " + nuovaCarta.getTerritorio().getNome() : ""));
                    FileServiceImpl.getInstance().writeLog("");
                } else {
                    OutputUtils.println("Il mazzo di carte è vuoto, non puoi ricevere una nuova carta.", OutputUtils.ANSI_RED, OutputUtils.ANSI_BOLD);
                }
    ...
    /**
     * Permette al giocatore di spostare armate tra territori adiacenti controllati e terminare il turno.
     *
     * @param giocatore il giocatore corrente.
     */
    private void spostamentoArmate(Giocatore giocatore) {
        ...
        OutputUtils.print("\nScelta: ", OutputUtils.ANSI_CYAN, OutputUtils.ANSI_BOLD);
        int indicePartenza = SingletonMain.getInstance().readIntegerUntilPossibleValue(IntStream.rangeClosed(1, territoriControllati.size()).boxed().toArray(Integer[]::new)) - 1;
        ...
        List<Territorio> territoriAdiacentiPosseduti = territorioPartenza.getTerritoriAdiacenti().stream().filter(t -> t.getGiocatore().equals(giocatore)).collect(Collectors.toList());
        ...
        OutputUtils.print("\nScelta: ", OutputUtils.ANSI_CYAN, OutputUtils.ANSI_BOLD);
        int indiceDestinazione = SingletonMain.getInstance().readIntegerUntilPossibleValue(IntStream.rangeClosed(1, territoriAdiacentiPosseduti.size()).boxed().toArray(Integer[]::new)) - 1;
        ...
        int armateDaSpostare = SingletonMain.getInstance().readIntegerUntilPossibleValue(IntStream.range(1, territorioPartenza.getNumeroArmate()).boxed().toArray(Integer[]::new));
        ...
        FileServiceImpl.getInstance().writeLog("Giocatore " + giocatore.getNome().toUpperCase() + " ha spostato " + armateDaSpostare + " armate da " + territorioPartenza.getNome() + " a " + territorioDestinazione.getNome());
        FileServiceImpl.getInstance().writeLog(""); // Riga vuota per separare le azioni
    }
     ...
     * @param gioco      l'oggetto Gioco corrente.
     */
    private void distribuzioneArmate(Giocatore giocatore, TurnState turnState, Gioco gioco) {
        ...
        if (gioco.getRoundCount() > 3) {
            if (haCombinazioneValida) {
                OutputUtils.printTurnHeader(giocatore);
                OutputUtils.println("\nHai una combinazione di 3 carte. Vuoi scambiarle per ottenere armate aggiuntive? (S/N)", OutputUtils.ANSI_BRIGHT_PURPLE, OutputUtils.ANSI_BOLD);
                OutputUtils.print("\nScelta: ", OutputUtils.ANSI_CYAN, OutputUtils.ANSI_BOLD);
                String rispostaScambio = SingletonMain.getInstance().readString();
        ...
        while (armateDaDistribuire > 0) {
            ...
            OutputUtils.println(giocatore.getNome().toUpperCase() + " riceve " + turnState.getArmateTotali() + " armate (Territori: " + turnState.getNumeroTerritori() + ", Continenti: " + turnState.getNumeroContinenti() + ").\n", OutputUtils.ANSI_BRIGHT_GREEN, OutputUtils.ANSI_BOLD);
            ...
            OutputUtils.print("\nScelta: ", OutputUtils.ANSI_CYAN, OutputUtils.ANSI_BOLD);
            int indiceTerritorio = SingletonMain.getInstance().readIntegerUntilPossibleValue(IntStream.rangeClosed(1, giocatore.getTerritori_controllati().size()).boxed().toArray(Integer[]::new)) - 1;
            System.out.println();
    ...
    private void salvaEEsci(Gioco gioco) {
        try {
            AnsiConsole.out().print("\033[H\033[2J"); // master 4
            AnsiConsole.out().flush();
