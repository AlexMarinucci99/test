...
import it.univaq.disim.lpo.risiko.core.datamodel.Carta;
...
public class GiocatoreServiceImpl implements GiocatoreService {
    ...
    private List<String> coloriDisponibili = new ArrayList<>(
            Arrays.asList("rosso", "blu", "verde", "giallo", "nero", "bianco"));
    ...
    public List<Giocatore> creaGiocatori(int numeroGiocatori) {
        ...
        for (int i = 1; i <= numeroGiocatori; i++) {
            ...
            while (nome == null || nome.trim().isEmpty()) {
                ...
                nome = SingletonMain.getInstance().readString();
            }
    ...
    public List<Giocatore> lancioDadiPerPrimoGiocatore(List<Giocatore> giocatori) {
        ...
        OutputUtils.println("\nL'ordine dei giocatori è: " +
                ordineGiocatori.stream().map(Giocatore::getNome).collect(Collectors.joining(", ")).toUpperCase(),
    ...
    /**
     * Mostra l'obiettivo al giocatore con un timer per impedire agli altri di guardare.
     *
    ...
    public boolean possiedeAlmenoUnaCombinazioneValida(Giocatore giocatore) {
        ...
        boolean jollyPiuDueUguali = conteggio.getOrDefault(Carta.TipoCarta.JOLLY, 0) >= 1 &&
                                    conteggio.entrySet().stream()
                                            .anyMatch(e -> e.getKey() != Carta.TipoCarta.JOLLY && e.getValue() >= 2);
    ...
    public boolean isCombinazioneSpecificataValida(List<Carta> carteSelezionate) {
        ...
        Map<Carta.TipoCarta, Long> conteggio = carteSelezionate.stream()
    ...
    public int scambiaCartePerArmate(Giocatore giocatore, Gioco gioco) {
        ...
        for (int i = 0; i < carteGiocatore.size(); i++) {
            ...
            OutputUtils.println((i + 1) + ") " + carta.getTipo() +
                    (carta.getTerritorio() != null ? " - " + carta.getTerritorio().getNome() : ""),
                    OutputUtils.ANSI_BOLD);
        }
        ...
        while (true) {
            ...
            OutputUtils.print("\nScelta: ", OutputUtils.ANSI_CYAN, OutputUtils.ANSI_BOLD);
            String input = SingletonMain.getInstance().readString();
            ...
            for (String indiceStr : indiciString) {
                try {
                    ...
                    if (!uniqueIndices.add(indice)) {
                        OutputUtils.print("\nHai selezionato lo stesso territorio più volte.", OutputUtils.ANSI_RED, OutputUtils.ANSI_BOLD);
                        inputValido = false;
                        break; // Esce dal loop degli indici
                    }
